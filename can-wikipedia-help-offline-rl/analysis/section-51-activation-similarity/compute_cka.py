...
def run_cka(
    ...
    for env_name in env_name_list:
        ...
        variant = generate_variant(
            epoch1, path_to_model_checkpoint, model1, env_name, seed, dataset_name
        )

        if no_context:
            variant["load_checkpoint"] = (
                False
                if epoch1 == 0
                else f"{path_to_model_checkpoint}/{model1}_{dataset_name}_{env_name}_{seed}_K1/model_{epoch1}.pt"
            )
        ...
        for _ in range(2):
            ...
            variant = generate_variant(
                epoch2, path_to_model_checkpoint, model2, env_name, seed, dataset_name
            )
            ...
            if no_context:
                variant["load_checkpoint"] = (
                    False
                    if epoch2 == 0
                    else f"{path_to_model_checkpoint}/{model2}_{dataset_name}_{env_name}_{seed}_K1/model_{epoch2}.pt"
                )
        ...
        if block:
            for reward_state_action in reward_state_action_list:
                ...
                np.save(
                    f"{path_to_save_cka}/cka_block_{epoch1}_{epoch2}_{model1}{model2}_{env_name}_{dataset_name}_{seed}_{reward_state_action}.npy",
                    cka_matrix,
                )
                plot_cka(
                    path_to_save_figure,
            ...
            for reward_state_action in reward_state_action_list:
                ...
                for key_1, act_1 in tqdm(activation_list[0].items()):
                    ...
                    for key_2, act_2 in activation_list[1].items():
                        cka = compute_cka(
                            act_1, act_2, reward_state_action, timestep=-1
                        )
                        cka_list.append(cka)
                    cka_matrix.append(cka_list)
                cka_matrix = np.array(cka_matrix)
                ...
                np.save(
                    f"{path_to_save_cka}/cka_{epoch1}_{epoch2}_{model1}{model2}_{env_name}_{dataset_name}_{seed}_{reward_state_action}.npy",
                    cka_matrix,
                )
                plot_cka(
                    path_to_save_figure,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--path_to_save_cka", "-psc", type=str, default="results")
